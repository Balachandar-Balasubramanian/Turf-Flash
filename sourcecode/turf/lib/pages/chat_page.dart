...
class ChatPageState extends State<ChatPage> {
  ...
  void initState() {
    ...
    recent = authProvider.getPref(FirestoreConstants.recentChat);
    getPeerRecent();
  ...
  Future<void> getPeerRecent() async {
    peerRecent = await authProvider.getRecent(widget.arguments.peerId);
  }
  ...
  void onSendMessage(String content, int type) {
    if (content.trim().isNotEmpty) {
      ...
      if (!recent.contains(widget.arguments.peerId)) {
        print(currentUserId);
        print(widget.arguments.peerId);
        recent.add(widget.arguments.peerId);
        authProvider.updateDataFirestore(currentUserId, recent);
        // authProvider.updateDataFirestore(currentUserId, recent);
        authProvider.setPref(FirestoreConstants.recentChat, recent);
      }
      if (!peerRecent.contains(currentUserId)) {
        peerRecent.add(currentUserId);
        authProvider.updateDataFirestore(widget.arguments.peerId, peerRecent);
        // authProvider.setPref(FirestoreConstants.recentChat, peerRecent);
      }
  ...
  Widget buildInput() {
    return Container(
      ...
      decoration: BoxDecoration(
          border: Border(
              top:
                  BorderSide(color: ColorConstants.secondaryColor, width: 0.5)),
          color: ColorConstants.secondaryColor),
    );
  }
